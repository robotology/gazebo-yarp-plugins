...
void PedestrianSFMPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
    ...
    std::string port_name = "/gazebo/" + std::string(this->actor->GetName()) + "/rpc";
    port.open(port_name);

    thr = std::thread(&PedestrianSFMPlugin::rpcListener, this);

    //////////////
}

/////////////////////////////////////////////////
void PedestrianSFMPlugin::rpcListener()
{
    while (true) {
        ...
        Bottle response;
        port.read(cmd, true);
        const char* cmd_string = cmd.toString().c_str();
        printf("MESSAGE: %s\n", cmd_string);
        std::cout << std::string(this->actor->GetName()) << " got message " << cmd.toString() << std::endl;

        if (!cmd.get(0).toString().compare("clear")) {
            std::cout << std::string(this->actor->GetName()) << " PedestrianSFMPlugin: clear goals called!\n";
            this->sfmActor.goals.clear();
            response.addString("ack");
        } else if (!cmd.get(0).toString().compare("cycle on")) {
            std::cout << std::string(this->actor->GetName()) << " PedestrianSFMPlugin: cycle goals called!\n";
            this->sfmActor.cyclicGoals = true;
            response.addString("ack");
        } else if (!cmd.get(0).toString().compare("cycle off")) {
            std::cout << std::string(this->actor->GetName()) << " PedestrianSFMPlugin: uncycle goals called!\n";
            this->sfmActor.cyclicGoals = false;
            response.addString("ack");
        } else if (!cmd.get(0).toString().compare("add")) {
            std::cout << std::string(this->actor->GetName()) << " PedestrianSFMPlugin: add goal called!\n";
            sfm::Goal goal;
            goal.center.set(atof(cmd.get(1).toString().c_str()), atof(cmd.get(2).toString().c_str()));
            goal.radius = 0.3;
            this->sfmActor.goals.push_back(goal);
            response.addString("ack");
        } else if (!cmd.get(0).toString().compare("help")) {
            response.addVocab32(Vocab32::encode("many"));
            response.addString("Available commands are:");
            response.addString("add x y   - add a new goal");
            response.addString("clear     - clear all goals");
            response.addString("cycle on  - cycle between goals");
            response.addString("cycle off - do not cycle goals");
        }
