...
std::string WorldInterfaceServerImpl::loadModelFromFileWithPose(const std::string& filename, const GazeboYarpPlugins::Pose& pose, const std::string& object_name, const double timeout)
{
  return proxy->loadModelFromFileWithPose(filename, pose, object_name, timeout);
}
